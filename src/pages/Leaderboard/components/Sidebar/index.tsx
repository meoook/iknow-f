...
export default function SidebarWins({ tag, period }: SidebarProps) {
  ...
  return (
    <aside className={s.sidebar}>
      <h2 className='pv-2'>Крупнейшие выигрыши за {periodTitle}</h2>
...
const WinList = ({ tag, period }: SidebarProps) => {
  ...
  return (
    <div className='column gap-4 pv-1 text-sm'>
      {data.map((win, idx) => (
        <div key={win.id} className='row center gap-2 pv-2 bd-b'>
          ...
          <div className='grow w-0'>
            <div className={s.top}>
              ...
              <Link to={`/prediction/${win.prediction.id}`} className='ellipsis secondary h-brand'>
